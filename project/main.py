...
def change_image(sender, app_data, user_data):
    ...
    if project_info.frame_path :
        ...
        if(frame_num3<project_info.frame_max_index):
            ...
            width, height, _, data1 = dpg.load_image(img1_path)
            width, height, _, data2 = dpg.load_image(img2_path)
            width, height, _, data3 = dpg.load_image(img3_path)
            if os.path.exists(img1_path):
                # width, height, _, data = dpg.load_image(img1_path)
                # for i in range(0, 80 * 80 * 4):
                #     test1[i] = data[i]
                dpg.set_value("texture_tag1", data1)
            ...
            if os.path.exists(img2_path):
                # width, height, _, data1 = dpg.load_image(img2_path)
                dpg.set_value("texture_tag2", data2)
            ...
            if os.path.exists(img3_path):
                # width, height, _, data = dpg.load_image(img3_path)
                dpg.set_value("texture_tag3", data3)
        ...
        with dpg.group(horizontal=True):
            ...
            with dpg.child_window(
                ...
                with dpg.texture_registry(show=False):
                    dpg.add_raw_texture(width, height, test1, tag="texture_tag1")
                    dpg.add_raw_texture(width, height, test2, tag="texture_tag2")
                    dpg.add_raw_texture(width, height, test3, tag="texture_tag3")
