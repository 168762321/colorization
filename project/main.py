...
def _delete_task(sender, app_data, user_data):
    ...
    try:
        if delete_id:
            info = session.query(Project).filter(Project.id==delete_id).first()
            ...
            # 删除文件
            delete_task_all_files(info.project_path)
            print("delete all task files")
...
#删除任务同时删除文件夹下全部内容
def delete_task_all_files(file_path):
    for i in os.listdir(file_path):
        ...
        if os.path.isfile(file_data) == True:#os.path.isfile判断是否为文件,如果是文件,就删除.如果是文件夹.递归给自己.
            ...
            delete_task_all_files(file_data)
    os.rmdir(file_path)


#导入视频回调
def video_file_select_callback(sender, app_data):
    input_task_name = dpg.get_value('task_name_input')
    # print(input_task_name)
    project_path = os.path.join(os.getcwd(), "task")#创建文件夹
    project_path = os.path.join(project_path, input_task_name)
    create_file(project_path)
    video_path = app_data['file_path_name']
    frame_path = os.path.join(os.getcwd(), "task")
    frame_path = os.path.join(frame_path, input_task_name)
    ...
    save_p_info= Project(name=input_task_name,project_path = project_path ,video_path= video_path,
    ...
    session.commit()
    p_task_id = session.query(Project.id).filter(Project.name == input_task_name).all()
    global task_id
    task_id = p_task_id[0][0]
    info = "创建任务成功，名称:"+str(input_task_name)
    log.write(info)
    print('任务创建完成')
    print('name:{},project_path:{},video_path:{},video_fps:{},frame_path:{},frame_max_index:{},create_time:{}'.format(input_task_name,project_path,video_path,video_fps,frame_path,frame_max_index,sql_time))
    Primary_Window()
...
def Colorization():
    ...
    with dpg.window(label="Colorization Window",width=800,height=600,id='Colorization Window'):
        ...
        with dpg.group(horizontal=True):
            dpg.add_button(label="编辑创建任务",callback=lambda: dpg.show_item("video_file_dialog_tag"))
        ...
        with dpg.table(label='task_list',header_row=True, resizable=True, policy=dpg.mvTable_SizingStretchProp,
            ...
            for i in range(0,table_num):
                with dpg.table_row():
                    ...
                    # dpg.add_button(label=f"{name_list[i][1]}" , user_data=name_list[i][0] , callback=_log,)  #回调没写，创建下一级页面，传递当前的user_data 给数据库。
    ...
    with dpg.window(label=win_name,width=800,height=800,id='Primary Window'):
        ...
        with dpg.menu_bar():
            ...
            with dpg.menu(label="导出"):
                dpg.add_menu_item(label="导出", callback=_log)
