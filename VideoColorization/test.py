...
import VideoColorization.lib.TestTransforms as transforms
...
from VideoColorization.models.NonlocalNet import VGG19_pytorch, WarpNet
from VideoColorization.utils.util import (batch_lab2rgb_transpose_mc, folder2vid, mkdir_if_not,
                        save_frames, tensor_lab2rgb, uncenter_l)
from VideoColorization.utils.util_distortion import CenterPad, Normalize, RGB2Lab, ToTensor
...
def colorize_video(image_size, frame_propagate, frame_list, reference_file, output_path, nonlocal_net, colornet, vggnet):
    ...
    for index, frame_file in enumerate(tqdm(frame_list)):
        frame1 = Image.open(frame_file)
        ...
        # save the frames
        save_frames(IA_predict_rgb, output_path, int(frame_file.split("/")[-1].split(".")[0]))
...
def load_models(root_dir):
    ...
    vggnet.load_state_dict(torch.load(
                            os.path.join(root_dir, "data/vgg19_conv.pth")))
    for param in vggnet.parameters():
        param.requires_grad = False

    checkpoints_dir = os.path.join(root_dir, "checkpoints")
    nonlocal_test_path = os.path.join(checkpoints_dir,
                                    "video_moredata_l1/nonlocal_net_iter_76000.pth")
    color_test_path = os.path.join(checkpoints_dir,
                                    "video_moredata_l1/colornet_iter_76000.pth")
